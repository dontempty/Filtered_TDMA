...
void SolveTheta::run(std::vector<double>& theta) {
    ...
    FilteredTDMA solver_x(ny1-2, nx1-2, cx.myrank, cx.nprocs, cx.comm,
                          cx.west_rank, cx.east_rank, eps_c);
    FilteredTDMA solver_y(nx1-2, ny1-2, cy.myrank, cy.nprocs, cy.comm,
                          cy.west_rank, cy.east_rank, eps_c);
    FilteredTDMA solver_z(nx1-2, nz1-2, cz.myrank, cz.nprocs, cz.comm,
    ...
    for (int t_step = 0; t_step < max_iter; ++t_step) {
        ...
        for (k = 1; k < nz1-1; ++k) {
            ...
            for (j = 1; j < ny1-1; ++j) {
                ...
                for (i = 1; i < nx1-1; ++i) {
                    auto sx = compute_stencil_cycl(dt, sub_.dmx_sub[i],
        ...
        for (j = 1; j < ny1-1; ++j) {
            for (k = 1; k < nz1-1; ++k) {
                ...
                solver_z.A_rho()[k] = sz.a / (1.0 - sz.b);
                solver_z.C_rho()[k] = sz.c / (1.0 - sz.b);
            }
            solver_z.solve_filtered_v1(Azz.data(), Bzz.data(), Czz.data(), Dzz.data());
            for (k = 1; k < nz1-1; ++k)
                for (i = 1; i < nx1-1; ++i)
        ...
        for (k = 1; k < nz1-1; ++k) {
            for (j = 1; j < ny1-1; ++j) {
                ...
                solver_y.A_rho()[j] = sy.a / (1.0 - sy.b);
                solver_y.C_rho()[j] = sy.c / (1.0 - sy.b);
            }
            solver_y.solve_filtered_v1(Ayy.data(), Byy.data(), Cyy.data(), Dyy.data());
            for (j = 1; j < ny1-1; ++j)
                for (i = 1; i < nx1-1; ++i)
        ...
        for (k = 1; k < nz1-1; ++k) {
            for (i = 1; i < nx1-1; ++i) {
                ...
                solver_x.A_rho()[i] = sx.a / (1.0 - sx.b);
                solver_x.C_rho()[i] = sx.c / (1.0 - sx.b);
            }
            solver_x.solve_cycl_filtered_v1(Axx.data(), Bxx.data(), Cxx.data(), Dxx.data());
            for (i = 1; i < nx1-1; ++i)
                for (j = 1; j < ny1-1; ++j)
