...
private:
    ...
    /// Single bidirectional neighbour exchange of both interfaces' reduced rows
    /// (my row N-1 C,D -> right ; my row 0 A,D -> left), one comm round. Shared
    /// by every multirank solve; the profile variant wraps it with timing.
    void exchange_interfaces_(double* A, double* C, double* D);
