...
#SBATCH -n 2
#SBATCH --gres=gpu:2
#SBATCH -o /scratch/x3319a05/Filtered_TDMA/Report/order_gpu2_v100nv8_%j.out
#SBATCH -e /scratch/x3319a05/Filtered_TDMA/Report/order_gpu2_v100nv8_%j.out
...
echo "host=$(hostname)  date=$(date '+%F %T')   (MERGED 1-comm filtered, npz=2)"
nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -2 | tr '\n' ' '; echo
echo "expect: L2 == CPU/1-GPU values, ratio->4, order->2, [rho]=0.25"

for backend in ${BACKENDS}; do
    echo ""
    printf "  %-12s %5s %5s %18s %8s %8s\n" backend nx Nt L2_error ratio order
    PREV=""
...
EOF
        ...
        printf "  %-12s %5s %5s %18s %8s %8s (rc=%d)\n" "${backend}" "${nx}" "${NT}" "${L2}" "${RATIO}" "${ORDER}" "${RC}"
        [ "${L2}" != "FAIL" ] && PREV="${L2}"
        [ "${L2}" == "FAIL" ] && { echo "    --- tail ---"; tail -15 "${LOG}" | sed 's/^/    /'; }
    done
done
rm -rf "${TMP}"
echo ""; echo "[done]"
