...
# Separate output dirs (*_h200_222_run3) so this does not clobber any
# other job's results (including the other two run instances).
# Submit:  sbatch apps/channel_gpu/sbatch_re550_filtered_h200_222_run3.sh
set -u
...
mkdir -p log statistics/re550_f_h200_222_run3 instant/re550_f_h200_222_run3 \
         restart_out/re550_f_h200_222_run3 restart_in/re550_f_h200_222_run3

rm -f instant/re550_f_h200_222_run3/* statistics/re550_f_h200_222_run3/* restart_out/re550_f_h200_222_run3/*

echo "=== channel_gpu Re550 FILTERED run3 (np=2x2x2=8) on $(hostname)  [$(date '+%F %T')] ==="
echo "  bin: ${BIN}"
...
mpirun --bind-to none --mca pml ucx --mca osc ucx --mca coll ^hcoll \
       -np 8 "${BIN}" input/PARA_INPUT_re550_filtered_h200_222_run3.dat
rc_f=$?
t1=$(date +%s)
echo "[filtered run3 done] rc=${rc_f}  wall=$((t1-t0))s  [$(date '+%F %T')]"
