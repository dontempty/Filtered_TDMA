...
mkdir -p log statistics/re550_f instant/re550_f \
         restart_out/re550_f restart_in/re550_f

t0=$(date +%s)
mpirun --bind-to none -np 64 "${EXE}" input/PARA_INPUT_re550_filtered.dat 2>&1
t1=$(date +%s)
echo
echo "[done]  filtered wall-time (mpirun only) = $((t1 - t0)) s"
ls -la statistics/re550_f/tdma_timing_rank*.csv 2>&1 | head
