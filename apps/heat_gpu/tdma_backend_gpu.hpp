...
public:
    ...
    /// Pure MPI communication time (ms) for the most recent solve() call.
    double last_comm_ms() const {
        if (kind_ == Kind::PASCAL) return pasc_->last_comm_ms();
        return filt_->last_comm_ms();
    }

    /// GPU kernel execution time (ms) for the most recent solve() call.
    double last_gpu_ms() const {
        if (kind_ == Kind::PASCAL) return pasc_->last_gpu_ms();
        return filt_->last_gpu_ms();
    }
