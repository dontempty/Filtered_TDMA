...
#SBATCH -o /scratch/x3319a05/Filtered_TDMA/apps/heat_gpu/log/comm_pure_%j.log
#SBATCH -e /scratch/x3319a05/Filtered_TDMA/apps/heat_gpu/log/comm_pure_%j.err
...
nvcc ${NVCC_FLAGS} -I${INCDIR} \
    ...
    -c -o build_sm90/obj/pascal/pascal_tdma_many_cuda.o 2>&1

nvcc ${NVCC_FLAGS} -I${INCDIR} \
    libs/filtered_tdma/filtered_tdma_cuda.cu \
    -c -o build_sm90/obj/ftdma/filtered_tdma_cuda.o 2>&1

# Also recompile solve_theta.cu (new event columns)
nvcc ${NVCC_FLAGS} -I${INCDIR} \
    apps/heat_gpu/solve_theta.cu \
    -c -o build_sm90/obj/exgpu_solve_theta.o 2>&1
...
mpicxx -O3 -std=c++17 -fPIC -march=x86-64-v3 \
    ...
    -L${CUDA_LIB} -lcudart -lstdc++

echo "=== Build done: $(ls -lh build_sm90/bin/heat_gpu.out) ==="
strings build_sm90/bin/heat_gpu.out | grep "tdma_z_gpu" | head -1
...
export TIMING_CSV=/tmp/sanity_comm_pure.csv
mpirun ${MPI_FLAGS} --host ${NODE}:8 -np 2 build_sm90/bin/heat_gpu.out \
    apps/heat_gpu/inputs/scaling/strong_128x128x1024_np2_filtered_v2_rho025.txt \
    2>&1 | grep -E "backend\]|Tmax\]" || true
echo "--- Sanity: tdma_z_comm, tdma_z_gpu ---"
grep -E "tdma_z_comm|tdma_z_gpu" /tmp/sanity_comm_pure.csv | head -4
