...
#SBATCH --comment etc
#SBATCH -o /scratch/x3319a05/Filtered_TDMA/apps/heat_gpu/log/scaling_h200_1comm_%j.out
#SBATCH -e /scratch/x3319a05/Filtered_TDMA/apps/heat_gpu/log/scaling_h200_1comm_%j.out
# GPU strong+weak scaling np=2,4,8 — merged 1-comm FilteredTDMA vs PaScaL.
# Rebuilds heat_gpu (sm_90=H200) IN-JOB from current source into a per-arch
# BUILDDIR (build_sm90) so it never races the shared build/ or the v100 job.
set -u
...
echo "=== [BUILD] heat_gpu sm_90 → ${BD}  [$(date '+%F %T')] ==="
module purge >/dev/null 2>&1
module load gcc/15.2.0 mpi/openmpi-4.1.8 nvhpc/25.11_cuda12 >/dev/null 2>&1
( cd "${PROJ}" && rm -rf "${BD}" && USE_CUDA=1 CUDA_ARCH=90 make heat_gpu BUILDDIR="$(basename "${BD}")" ) 2>&1 | tail -6
